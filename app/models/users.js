...
var UsersSchema = new Schema({
    ...
    email: { type: String, default: '',validate :  validators.isEmail() },//이메일
    phone: { type: String , validate: { //폰번호
...
UsersSchema.statics = {
    ...
    load: function (options, cb) { //options == 조건절 의미.
        console.log('-------models/users-------Start')
        //options.select = options.select || 'name email phone';
        console.log('select : ' , options.select);
        this.findOne(options.criteria)
            .select(options.select)//보여줄려는 컬럼 선택
            .exec(cb);
        console.log('-------models/users-------End')
    }
}
